...
test.describe('하위필지 선택 (SAMPL-1-159)', () => {
    ...
    test('실제 클릭 경로 — 하위필지 추가·배정·등록이 저장까지 이어진다', async ({ page }) => {
        ...
        // ⚠️ **수정 왕복은 여기서 검증할 수 없다.** 실측으로 확인한 사실:
        //    필지 카드는 첫 작물을 인라인 입력으로 보여주고 배지는 `slice(1)`이므로
        //    (`_renderAdditionalCrops`) **배정 배지·드롭다운이 의미 있는 것은 작물 2개 이상**이다.
        //    그런데 작물 2개는 분할모드 저장이 되고, 그 그룹수정 경로는 **이 티켓과
        //    무관한 선재 결함으로 crops를 잃는다** (실측: 저장 시 crops 1개 → 수정 화면에서 0개).
        //    `populateFormForEdit`·`populateFormForGroupEdit`·`soil-log-record.js`는
        //    이 변경에서 전혀 건드리지 않았다(`git diff origin/main` 확인).
        //    → **SAMPL-1-160**으로 분리했다. 그 티켓이 닫히면 이 테스트에 수정 왕복을 잇는다.
        //
