...
// header:1 형태(2차원 배열) fixture — 제목 1행 + 헤더 1행 + 데이터
// SAMPL-1-128: 실제 농진청 파일 헤더('비료사용추천 발급여부'/'작물목록 표시여부')를
// 그대로 사용해 컬럼명 접두어가 있어도 파싱되는지(정확일치가 아닌 부분포함) 검증한다.
...
describe('CropDataLoader.mapGroupToCategory — 그룹→카테고리 축약', () => {
    ...
    it('매핑에 없는 그룹은 그대로 통과(pass through)', () => {
        expect(Loader.mapGroupToCategory('과일류')).toBe('과일류')
...
describe('CropDataLoader.parseCropRows — happy path', () => {
    ...
    // SAMPL-1-128 회귀 방지: 실제 농진청 파일 레이아웃(제목행 + 접두어 헤더 + 빈행 + 데이터).
    // 파서가 '작물목록 표시여부'를 '표시여부' 부분포함으로 매칭해야 통과한다.
