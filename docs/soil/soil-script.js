...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ========================================
    // 엑셀 파일 가져오기 기능 (ExcelImportManager 사용)
    // ========================================
    const excelImporter = new ExcelImportManager({
        ...
        buildRecord: (getVal, parseExcelDate, common, rowIdx) => {
            ...
            return {
                ...
                parcelIndex: 0, // postBuildRecords에서 설정
                totalParcels: 0,
                parcels: [{
                    id: crypto.randomUUID(),
                    lotAddress,
        ...
        skipRowCheck: (record, rowIdx) => {
            if (!record.lotAddress && record.cropsDisplay === '-' && !record.name) {
                return `행 ${rowIdx + 2}: 필지주소, 작물, 성명이 모두 비어 있어 건너뜁니다.`;
            }
            return null;
        },
        ...
        onImportComplete: (records) => {
            ...
            saveLogs();
            renderLogs(sampleLogs);
            log(`📊 엑셀 가져오기 완료: ${records.length}건`);
        }
    });
    excelImporter.init();
