...
설정 페이지에서 인증 파일을 등록해주세요.`);return}m.classList.add("syncing");const t=new Date().getFullYear(),n=[],s=u.length*(t-I+1);let e=0;D(u);try{for(let a=0;a<u.length;a++){const i=u[a];let g=0,$=[];P(a,"syncing","동기화 중...");for(let r=I;r<=t;r++){try{const l=await window.firestoreDb.getAll(i.type,r),b=`${i.storagePrefix}_${r}`;l&&l.length>0&&(localStorage.setItem(b,JSON.stringify(l)),g+=l.length,$.push({year:r,count:l.length}))}catch(l){(((S=window.logger)==null?void 0:S.error)||console.error)(`${i.name} ${r}년 동기화 오류:`,l)}e++,T(e,s,`${i.name} ${r}년...`)}const C=g>0?`${g}건 완료`:"데이터 없음";P(a,g>0?"success":"",C),n.push({type:i.type,name:i.name,icon:i.icon,totalCount:g,yearsWithData:$,success:!0})}A(n)}catch(a){(((L=window.logger)==null?void 0:L.error)||console.error)("동기화 오류:",a),alert("동기화 중 오류가 발생했습니다: "+a.message)}finally{m.classList.remove("syncing")}}function D(t){let n=`
    ...
    `;t.forEach((s,e)=>{n+=`
        ...
        `}),M.innerHTML=n,d.classList.add("show")}function T(t,n,s){const e=Math.round(t/n*100),o=document.getElementById("syncProgressFill"),c=document.getElementById("syncProgressLabel"),y=document.getElementById("syncProgressPercent");o&&(o.style.width=`${e}%`),c&&(c.textContent=s),y&&(y.textContent=`${e}%`)}function P(t,n,s){const e=document.getElementById(`syncItem${t}`),o=document.getElementById(`syncStatus${t}`);e&&(e.className=`sync-result-item ${n}`),o&&(o.className=`sync-result-count ${n}`,o.textContent=s)}function A(t){let n=0,s="";t.forEach(e=>{n+=e.totalCount;const o=e.totalCount>0?"success":"";e.yearsWithData.length>0&&e.yearsWithData.map(c=>`${c.year}년: ${c.count}건`).join(", "),s+=`
    ...
    `,M.innerHTML=s,d.classList.add("show")}m==null||m.addEventListener("click",B);window.DOMPurify=x;
