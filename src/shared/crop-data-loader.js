/**
 * @fileoverview 작물 데이터 자체 업로드/로딩 모듈 (SAMPL-1-126)
 ...
 * cropData.js 생성 시점에 적용된 규칙과 동일하게 복제(드리프트 방지 위해 규칙 고정).
...
function parseCropRows(rows) {
    ...
    // 접두어가 붙어 있어, 핵심 키워드 포함 여부로 매칭한다(SAMPL-1-128).
    const COLS = {
        code: ['작물코드'],
        name: ['작물명'],
        show: ['표시여부'],
...
async function saveCropDataUpload(parsedArray, versionLabel) {
    ...
    // Firestore 백업(best-effort)
    try {
        await window.firestoreDb?.saveCropDataConfig?.(parsedArray, versionLabel);
    } catch (e) {
