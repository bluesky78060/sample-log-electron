...
class HeuktoramManager {
    ...
    parseLotAddress(lotAddress) {
        ...
        // 시군구 → 시도 추론 (모듈 상수 + bonghwaData.js 동적 반영)
    ...
    getBeforeAfter(usageCode) {
        ...
        // bulkBeforeAfterSelect 값('N'/'Y')을 AN3/AO3 범례 표기('전-N'/'후-Y')와 일치시켜 출력
    ...
    async exportToHeuktoram() {
        ...
        try {
            ...
            ws['!rows'] = ws['!rows'] || [];
            // 실물 서식 값 그대로 (SAMPL-1-145에서 전수 대조).
            ws['!rows'][0] = { hpt: 31.5 };   // 1행 제목
            ws['!rows'][1] = { hpt: 309.75 }; // 2행 안내문 (wrapText 18줄)
     ...
     *    분리 저장소에서 실제로 시료번호·암모니아태질소 입력이 막혔던 전례가 있다.
     */
    buildDataValidations(dataRowCount) {
        ...
        // 실물은 열별로 미세하게 다르다 — F는 3행부터, G는 65533에서 끝,
        // U는 헤더행(U3:U4)에도 별도 검증이 있다. 우리는 5행부터 균일하게 둔다.
        // endRow는 실물보다 넓지만 startRow는 좁다(헤더행 제외) — 사용자가 헤더에
        // 입력하지 않으므로 무해한 단순화다. "상위집합"이 아니다 (SAMPL-1-145).
        ...
        return [
            ...
            {
                // G열: 시행(재배)전후 — AN3='전-N', AO3='후-Y' 범례 텍스트와 동일
    ...
    buildWorksheetData(rows) {
        ...
        // 2행: 안내문 (A2:AH2 병합) - 흙토람 신규 서식(2026-07-14 개인정보 필드 삭제) 기준
        const row2 = new Array(41).fill('');
        ...
        // 3행: 대분류 헤더 (흙토람 신규 서식 기준 A~AH = 34열 데이터, AJ~AO = 코드 범례)
        const row3 = new Array(41).fill('');
        row3[0] = " *필지구분";
        row3[1] = " *채취년도";
        row3[2] = "*분석의뢰일\n(접수일자)";
        row3[3] = " *경지구분";   // 소분류: 1차, 2차 (가로 병합)
        // [4]: 경지구분 2차 (가로 병합)
        row3[5] = "*용도구분";    // 소분류: 코드, 시행(재배)전후 (가로 병합)
        // [6]: 용도구분 시행(재배)전후 (가로 병합)
        row3[7] = "*시료번호";
        row3[8] = "*대상지 주소"; // 소분류: 시도, 시군구, 읍면동, 리 (가로 병합)
        // [9~11]: 가로 병합
        row3[12] = "지번 구분";
        row3[13] = "*지번";       // 소분류: 지번1, 지번2 (가로 병합)
        // [14]: 가로 병합
        row3[15] = "주소매핑여부";
        row3[16] = "기타주소";
        row3[17] = "*면적(㎡)";
        ...
        row3[32] = "양이온\n치환용량";
        row3[33] = "암모니아태\n질소";
        // [34]: 빈 스페이서 컬럼 (신규 서식 실측 결과 존재)
        // 코드 범례 (AJ=35 ~ AO=40) — 흙토람 서식 원본과 동일
        row3[35] = "일반적인토양검정-0";
        row3[36] = "토양개량제 규산-1";
        row3[37] = "토양개량제 석회질-2";
        row3[38] = "녹비작물-3";
        ...
        for (const row of rows) {
            ...
            dataRow[5] = usageLabels[usageCode] || '일반적인토양검정-0';
    ...
    getColumnWidths() {
        return [
            ...
            { wch: 12 }, // [32] 양이온 치환용량
            { wch: 12 }, // [33] 암모니아태 질소
            // 실물은 스페이서와 범례가 숨김이다. 안 숨기면 사용자가 '일반적인토양검정-0'
            // 같은 범례를 입력할 데이터 열로 착각한다. 검증이 참조하므로 지우면 안 된다.
            { wch: 8, hidden: true },  // [34] 스페이서 (신규 서식 실측 빈 컬럼)
    ...
    applyHeaderStyles(ws, wsData) {
        const colCount = wsData[0]?.length || 41;
        ...
        const dataStyle = {
            ...
            // 날짜 칸을 고칠 때 Excel이 일련번호로 바꿔 업로드가 깨진다.
            numFmt: '@',
            alignment: { horizontal: 'center', vertical: 'center', wrapText: true },
            border: {
                top: { style: 'thin', color: { rgb: '808080' } },
        ...
        // 데이터 영역은 A~AH(0~33)만, 스페이서(34)·코드 범례 영역(AJ~AO=35~40)은 헤더 행에만 표시
        ...
        for (let c = 0; c < colCount; c++) {
            ...
            if (c < DATA_COL_END) {
                ...
                // 코드 범례 영역(AJ3~AO3): 배경/굵기/테두리 없이 일반 셀 (서식.xlsx 원본 기준)
                ws[cell3Addr].s = {
    ...
    applyHeaderMerges(ws) {
        const merges = [
            // 1행: 제목 A1:AH1 병합 (신규 서식 기준, 데이터 마지막 컬럼 33까지 — 실측: 검정자료엑셀서식(4).xlsx)
            { s: { r: 0, c: 0 }, e: { r: 0, c: 33 } },
            // 2행: 안내 A2:AH2 병합 (신규 서식 기준)
            { s: { r: 1, c: 0 }, e: { r: 1, c: 33 } },

            // 3행-4행 세로 병합 (단독 컬럼)
            { s: { r: 2, c: 0 }, e: { r: 3, c: 0 } },   // A3:A4 필지구분
            { s: { r: 2, c: 1 }, e: { r: 3, c: 1 } },   // B3:B4 채취년도
            { s: { r: 2, c: 2 }, e: { r: 3, c: 2 } },   // C3:C4 분석의뢰일
            { s: { r: 2, c: 7 }, e: { r: 3, c: 7 } },   // H3:H4 시료번호
            { s: { r: 2, c: 12 }, e: { r: 3, c: 12 } },  // M3:M4 지번 구분
            { s: { r: 2, c: 15 }, e: { r: 3, c: 15 } },  // P3:P4 주소매핑여부
            { s: { r: 2, c: 16 }, e: { r: 3, c: 16 } },  // Q3:Q4 기타주소
            { s: { r: 2, c: 17 }, e: { r: 3, c: 17 } },  // R3:R4 면적
            { s: { r: 2, c: 18 }, e: { r: 3, c: 18 } },  // S3:S4 토양검정일
            { s: { r: 2, c: 19 }, e: { r: 3, c: 19 } },  // T3:T4 작물명
            { s: { r: 2, c: 20 }, e: { r: 3, c: 20 } },  // U3:U4 성토여부
            { s: { r: 2, c: 21 }, e: { r: 3, c: 21 } },  // V3:V4 점토함량
            { s: { r: 2, c: 22 }, e: { r: 3, c: 22 } },  // W3:W4 pH
            { s: { r: 2, c: 23 }, e: { r: 3, c: 23 } },  // X3:X4 유기물
            { s: { r: 2, c: 24 }, e: { r: 3, c: 24 } },  // Y3:Y4 유효인산
            { s: { r: 2, c: 25 }, e: { r: 3, c: 25 } },  // Z3:Z4 교환성칼륨
            { s: { r: 2, c: 26 }, e: { r: 3, c: 26 } },  // AA3:AA4 교환성칼슘
            { s: { r: 2, c: 27 }, e: { r: 3, c: 27 } },  // AB3:AB4 교환성마그네슘
            { s: { r: 2, c: 28 }, e: { r: 3, c: 28 } },  // AC3:AC4 유효규산
            { s: { r: 2, c: 29 }, e: { r: 3, c: 29 } },  // AD3:AD4 전기전도도
            { s: { r: 2, c: 30 }, e: { r: 3, c: 30 } },  // AE3:AE4 석회소요량
            { s: { r: 2, c: 31 }, e: { r: 3, c: 31 } },  // AF3:AF4 질산태질소
            { s: { r: 2, c: 32 }, e: { r: 3, c: 32 } },  // AG3:AG4 양이온치환용량
            { s: { r: 2, c: 33 }, e: { r: 3, c: 33 } },  // AH3:AH4 암모니아태질소

            // 3행 가로 병합 (그룹 헤더)
            { s: { r: 2, c: 3 }, e: { r: 2, c: 4 } },    // D3:E3 경지구분
            { s: { r: 2, c: 5 }, e: { r: 2, c: 6 } },    // F3:G3 용도구분
            { s: { r: 2, c: 8 }, e: { r: 2, c: 11 } },   // I3:L3 대상지 주소
            { s: { r: 2, c: 13 }, e: { r: 2, c: 14 } },  // N3:O3 지번
        ];
