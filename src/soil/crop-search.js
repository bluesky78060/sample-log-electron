...
(function () {
    ...
    /** 목록에 한 번에 그리는 최대 건수 */
    // CROP_DATA는 801건이고 `기타` 318 · `과수` 170으로 **488건(61%)이 200 밖**이다.
    // 이 기능의 목적이 "미리 보지 않으면 무엇을 골라야 할지 모르는 이름"을 둘러보는 것인데
    // 상한 200은 카탈로그의 4분의 1만 보여준다 (코드 리뷰 지적). 전량이 들어가게 올린다.
    // 801행 × 3 span ≈ 2,400 엘리먼트로 모달 하나가 감당 가능하다.
    ...
    function filterCrops(crops, opts = {}) {
        ...
        // `typeof ... === 'number'`로 쓴다 — Number.isFinite는 TS가 타입을 좁히지 못해
        // @ts-check에서 'possibly undefined'가 난다. NaN/Infinity 배제는 그대로 유지한다.
