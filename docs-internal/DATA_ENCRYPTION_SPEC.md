...
> 주요 변경: 복합 키 기본화, AES-256-GCM, Web Crypto API, PBKDF2 600K
...
const CryptoUtils = (function () {
    ...
    function validatePassword(password) {
        ...
        if (password && password.length > 16) {
            errors.push('비밀번호는 16자 이하여야 합니다');
        }
...
| 5 | 키를 메모리에만 유지 (extractable: false) | Web Crypto API 모범 사례 |
